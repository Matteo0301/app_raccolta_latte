...
import 'package:app_raccolta_latte/origin_details/details_table.dart';
...
class OriginDetails extends StatefulWidget {
  const OriginDetails(
      ...
      required this.admin, required this.origin});
  final String title;
...
class OriginDetailsState extends State<OriginDetails> {
  ...
  Future<List<Collection>> getCollectionList() async {
    ...
    final res =
        (await getCollections(widget.username, widget.admin, startDate, endDate)).where((element) => element.origin == widget.origin).toList();
    return res;
  }
